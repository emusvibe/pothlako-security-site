...
<?php $query1 = mysqli_query($con,"select * from student_class_quiz where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ")or die(mysql_error());
...
$sqlp = mysqli_query($con,"SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'")or die(mysql_error());
...
<?php
	...
	mysqli_query($con,"UPDATE student_class_quiz SET `student_quiz_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'")or die(mysql_error());
?>
<script>
